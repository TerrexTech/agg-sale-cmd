	...
	for {
		...
		case eventResp := <-eventPoll.Delete():
			go func(eventResp *poll.EventResponse) {
				...
				kafkaResp := sale.Delete(mc.AggCollection, &eventResp.Event)
				if kafkaResp != nil {
		...
		case eventResp := <-eventPoll.Insert():
			go func(eventResp *poll.EventResponse) {
				...
				kafkaResp := sale.Insert(mc.AggCollection, &eventResp.Event)
				if kafkaResp != nil {
					eventPoll.ProduceResult() <- kafkaResp
				}
			}(eventResp)
		...
		case eventResp := <-eventPoll.Update():
			go func(eventResp *poll.EventResponse) {
				...
				kafkaResp := sale.Update(mc.AggCollection, &eventResp.Event)
				if kafkaResp != nil {
					eventPoll.ProduceResult() <- kafkaResp
				}
			}(eventResp)
